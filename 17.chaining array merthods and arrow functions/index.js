...
let replicants = people.filter(function(person){
    ...
    if(person.email.indexOf("@replicant.io") > -1){  //indexOf searches for the text that is passed in it's parameter and returns the index at which the text is found. if text is not found it return -1
        console.log(person.email.indexOf("@replicant.io"));
        return person
    }
})
...
let chainnames = people.filter(function(person){
