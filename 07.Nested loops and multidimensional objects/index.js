...
let chars2 = bigHero["characters"]; //this will work too
console.log("characters inside bigHero", bigHero.characters);
console.log("bigHero characters", bigHero["characters"]);
console.log("length", bigHero.characters.length); //also chars.length

for (let i = 0; i < bigHero.characters.length; i++) {
  console.log("bigHero.characters[i]", bigHero.characters[i]);

  for (item in bigHero.characters[i]) {
    //use for in loops for objects, forEach loop for arrays
    console.log("item", item);
    // item represents the keys in object
    console.log(bigHero.characters[i][item]);
    // bigHero.characters[i][item] represents values in keys
  }
}
